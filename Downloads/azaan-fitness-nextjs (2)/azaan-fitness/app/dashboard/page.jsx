...
export default function DashboardPage() {
  const { members, addMember, renewMember, deleteMember, resetToSample } = useMembers();
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div
        className="flex items-center justify-between rounded-xl px-4 py-2.5 border border-green-500/20"
      ...
      <div className="flex flex-col gap-3">
        {filteredMembers.length === 0 ? (
          ...
          filteredMembers.map((m) => (
            <MemberCard key={m.id} member={m} onRenew={renewMember} onDelete={deleteMember} />
          ))
        )}
      </div>
